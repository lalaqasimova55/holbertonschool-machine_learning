#!/usr/bin/env python3
"""
YOLOv3 object detection post-processing, image loading, preprocessing,
and visual display of detected bounding boxes.
"""
import cv2
import glob
import numpy as np
...
class Yolo:
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        ...
        for i in range(len(boxes)):
            ...
            # Draw bounding box (Blue line, thickness 2)
            cv2.rectangle(
                image,
                (x1, y1),
                (x2, y2),
            ...
            # Draw text (Red color, 5px above top-left corner)
            cv2.putText(
                image,
                label,
                (x1, y1 - 5),
